...
// Tahap V2.24 — Automation Center Data Integration (lihat DASHBOARD-V2-
// AUTOMATION-DATA.md).
//
...
// (Recent Activity), V2.22 (Upcoming Tasks) & V2.23 (Notifications). 4
// elemen baru ditambah sbg anak Automation Center, satu per fungsi
...
// tersedia/return `null`. 5 kartu Automation Center lama (autoBackup/
// monthlyReport/budgetReminder/vehicleServiceReminder/
// documentRenewalReminder, Tahap V2.13) TIDAK berubah.
//
// Fake DOM sama persis dgn tests/dashboard-v2-notifications-data.test.js
...
// sama sekali (mensimulasikan "adapter belum tersedia"). Automation
// Center harus tetap utuh & fallback ke placeholder, bukan error.
...
function getAutomationCenter(Shell) {
...
test('Automation Center: adapter tidak di-load sama sekali -> 4 elemen data summary tetap ada dgn fallback placeholder', () => {
  const { Shell } = loadShellOnly();
  const section = getAutomationCenter(Shell);
  assert.equal(section.children.length, 9, 'Automation Center harus 9 anak (5 lama V2.13 + 4 baru V2.24)');

  const finance = section.children.find((c) => c.id === 'dashboardV2AutomationFinanceData');
  assert.ok(finance, 'Finance summary tidak ditemukan');
  assert.equal(finance.className, 'dashboard-v2-automation-card');
  assert.match(finance.textContent, /placeholder/i);
  assert.match(finance.getAttribute('aria-label'), /placeholder/i);

  const vehicle = section.children.find((c) => c.id === 'dashboardV2AutomationVehicleData');
  assert.ok(vehicle, 'Vehicle summary tidak ditemukan');
  assert.equal(vehicle.className, 'dashboard-v2-automation-card');
  assert.match(vehicle.textContent, /placeholder/i);

  const family = section.children.find((c) => c.id === 'dashboardV2AutomationFamilyData');
  assert.ok(family, 'Family summary tidak ditemukan');
  assert.equal(family.className, 'dashboard-v2-automation-card');
  assert.match(family.textContent, /placeholder/i);

  const documentEl = section.children.find((c) => c.id === 'dashboardV2AutomationDocumentData');
  assert.ok(documentEl, 'Document summary tidak ditemukan');
  assert.equal(documentEl.className, 'dashboard-v2-automation-card');
  assert.match(documentEl.textContent, /placeholder/i);
});

test('Automation Center: 5 kartu lama (autoBackup/monthlyReport/budgetReminder/vehicleServiceReminder/documentRenewalReminder, Tahap V2.13) tidak berubah', () => {
  const { Shell } = loadShellOnly();
  const section = getAutomationCenter(Shell);
  const [autoBackup, monthlyReport, budgetReminder, vehicleServiceReminder, documentRenewalReminder] = section.children;
  assert.equal(autoBackup.id, 'dashboardV2AutomationCenterCardAutoBackup');
  assert.equal(autoBackup.tagName, 'BUTTON');
  assert.equal(autoBackup.disabled, true);
  assert.equal(monthlyReport.id, 'dashboardV2AutomationCenterCardMonthlyReport');
  assert.equal(monthlyReport.tagName, 'BUTTON');
  assert.equal(budgetReminder.id, 'dashboardV2AutomationCenterCardBudgetReminder');
  assert.equal(budgetReminder.tagName, 'BUTTON');
  assert.equal(vehicleServiceReminder.id, 'dashboardV2AutomationCenterCardVehicleServiceReminder');
  assert.equal(vehicleServiceReminder.tagName, 'BUTTON');
  assert.equal(documentRenewalReminder.id, 'dashboardV2AutomationCenterCardDocumentRenewalReminder');
  assert.equal(documentRenewalReminder.tagName, 'BUTTON');
});
...
test('Automation Center: fungsi adapter tersedia tapi return null -> fallback placeholder (tidak error)', () => {
  const { Shell } = loadShellOnly({
  ...
  for (const id of ['dashboardV2AutomationFinanceData', 'dashboardV2AutomationVehicleData', 'dashboardV2AutomationFamilyData', 'dashboardV2AutomationDocumentData']) {
...
test('Automation Center: getFinanceSummary() tersedia & ada data -> Finance summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Automation Center: getVehicleSummary() tersedia & ada data -> Vehicle summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Automation Center: getFamilySummary() tersedia & ada data -> Family summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Automation Center: getDocumentSummary() tersedia & ada data -> Document summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Integrasi sungguhan: adapter asli + D tiruan -> Automation Center menampilkan ringkasan dari D', () => {
  const D = minimalD({
...
test('Integrasi sungguhan: render() tetap idempotent (Automation Center 9 anak, tidak menumpuk saat dipanggil ulang)', () => {
  const D = minimalD();
  const { Shell } = loadShellWithRealAdapter(D);
  Shell.render();
  const section = getAutomationCenter(Shell);
...
test('Automation Center: 4 elemen data summary baru semuanya punya aria-label', () => {
  const { Shell } = loadShellOnly();
  const section = getAutomationCenter(Shell);
  for (const id of ['dashboardV2AutomationFinanceData', 'dashboardV2AutomationVehicleData', 'dashboardV2AutomationFamilyData', 'dashboardV2AutomationDocumentData']) {
    const el = section.children.find((c) => c.id === id);
...
test('dashboard-v2-shell.js (setelah V2.24) tetap tidak fetch/routing/FEATURE_REGISTRY, tidak membaca D langsung', () => {
...
test('dashboard-v2-data-adapter.js TIDAK diubah tahap ini (identik dgn isi baseline V2.16..V2.23)', () => {
...
test('Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19), Statistics Panel (V2.20), Recent Activity (V2.21), Upcoming Tasks (V2.22) & Notifications (V2.23) tidak ikut berubah oleh V2.24', () => {
  const { Shell } = loadShellOnly();
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  ...
  assert.equal(hero.children.length, 8, 'Hero harus tetap 8 anak seperti V2.17, tidak disentuh V2.24');
  assert.equal(cards.children.length, 8, 'Summary Cards harus tetap 8 anak seperti V2.18, tidak disentuh V2.24');
  assert.equal(moduleGrid.children.length, 10, 'Module Grid harus tetap 10 anak seperti V2.19, tidak disentuh V2.24');
  assert.equal(statisticsPanel.children.length, 8, 'Statistics Panel harus tetap 8 anak seperti V2.20, tidak disentuh V2.24');
  assert.equal(recentActivity.children.length, 9, 'Recent Activity harus tetap 9 anak seperti V2.21, tidak disentuh V2.24');
  assert.equal(upcomingTasks.children.length, 9, 'Upcoming Tasks harus tetap 9 anak seperti V2.22, tidak disentuh V2.24');
  assert.equal(notifications.children.length, 9, 'Notifications harus tetap 9 anak seperti V2.23, tidak disentuh V2.24');
});

for (const file of ['index.html', 'app_production.html']) {
  test(`${file}: tetap 0 markup Dashboard V2 (Automation Center Data Integration tetap self-mounting via JS)`, () => {
