...
// Tahap V2.23 — Notifications Data Integration (lihat DASHBOARD-V2-
// NOTIFICATIONS-DATA.md).
//
// Dashboard V2 mulai memakai dashboard-v2-data-adapter.js (V2.16) di
// Notifications, mengikuti pola persis Tahap V2.17 (Hero), V2.18
// (Summary Cards), V2.19 (Module Grid), V2.20 (Statistics Panel), V2.21
// (Recent Activity) & V2.22 (Upcoming Tasks). 4 elemen baru ditambah sbg
// anak Notifications, satu per fungsi adapter (getFinanceSummary/
// getVehicleSummary/getFamilySummary/getDocumentSummary), dgn fallback
// placeholder kalau adapter tidak tersedia/return `null`. 5 kartu
// Notifications lama (backup/pengeluaran/servis/laporan/sinkronisasi,
// Tahap V2.9) TIDAK berubah.
//
// Fake DOM sama persis dgn tests/dashboard-v2-upcoming-tasks-data.test.js
...
// sama sekali (mensimulasikan "adapter belum tersedia"). Notifications
...
function getNotifications(Shell) {
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  return main.children.find((c) => c.id === 'dashboardV2Notifications');
}
...
test('Notifications: adapter tidak di-load sama sekali -> 4 elemen data summary tetap ada dgn fallback placeholder', () => {
  const { Shell } = loadShellOnly();
  const section = getNotifications(Shell);
  assert.equal(section.children.length, 9, 'Notifications harus 9 anak (5 lama V2.9 + 4 baru V2.23)');

  const finance = section.children.find((c) => c.id === 'dashboardV2NotificationsFinanceData');
  assert.ok(finance, 'Finance summary tidak ditemukan');
  assert.equal(finance.className, 'dashboard-v2-notification-card');
  assert.match(finance.textContent, /placeholder/i);
  assert.match(finance.getAttribute('aria-label'), /placeholder/i);

  const vehicle = section.children.find((c) => c.id === 'dashboardV2NotificationsVehicleData');
  assert.ok(vehicle, 'Vehicle summary tidak ditemukan');
  assert.equal(vehicle.className, 'dashboard-v2-notification-card');
  assert.match(vehicle.textContent, /placeholder/i);

  const family = section.children.find((c) => c.id === 'dashboardV2NotificationsFamilyData');
  assert.ok(family, 'Family summary tidak ditemukan');
  assert.equal(family.className, 'dashboard-v2-notification-card');
  assert.match(family.textContent, /placeholder/i);

  const documentEl = section.children.find((c) => c.id === 'dashboardV2NotificationsDocumentData');
  assert.ok(documentEl, 'Document summary tidak ditemukan');
  assert.equal(documentEl.className, 'dashboard-v2-notification-card');
  assert.match(documentEl.textContent, /placeholder/i);
});

test('Notifications: 5 kartu lama (backup/pengeluaran/servis/laporan/sinkronisasi, Tahap V2.9) tidak berubah', () => {
  const { Shell } = loadShellOnly();
  const section = getNotifications(Shell);
...
test('Notifications: fungsi adapter tersedia tapi return null -> fallback placeholder (tidak error)', () => {
  const { Shell } = loadShellOnly({
  ...
  for (const id of ['dashboardV2NotificationsFinanceData', 'dashboardV2NotificationsVehicleData', 'dashboardV2NotificationsFamilyData', 'dashboardV2NotificationsDocumentData']) {
...
test('Notifications: getFinanceSummary() tersedia & ada data -> Finance summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Notifications: getVehicleSummary() tersedia & ada data -> Vehicle summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Notifications: getFamilySummary() tersedia & ada data -> Family summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Notifications: getDocumentSummary() tersedia & ada data -> Document summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Integrasi sungguhan: adapter asli + D tiruan -> Notifications menampilkan ringkasan dari D', () => {
  const D = minimalD({
...
test('Integrasi sungguhan: render() tetap idempotent (Notifications 9 anak, tidak menumpuk saat dipanggil ulang)', () => {
  const D = minimalD();
  const { Shell } = loadShellWithRealAdapter(D);
  Shell.render();
  const section = getNotifications(Shell);
...
test('Notifications: 4 elemen data summary baru semuanya punya aria-label', () => {
  const { Shell } = loadShellOnly();
  const section = getNotifications(Shell);
  for (const id of ['dashboardV2NotificationsFinanceData', 'dashboardV2NotificationsVehicleData', 'dashboardV2NotificationsFamilyData', 'dashboardV2NotificationsDocumentData']) {
    const el = section.children.find((c) => c.id === id);
...
test('dashboard-v2-shell.js (setelah V2.23) tetap tidak fetch/routing/FEATURE_REGISTRY, tidak membaca D langsung', () => {
...
test('dashboard-v2-data-adapter.js TIDAK diubah tahap ini (identik dgn isi baseline V2.16..V2.22)', () => {
...
test('Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19), Statistics Panel (V2.20), Recent Activity (V2.21) & Upcoming Tasks (V2.22) tidak ikut berubah oleh V2.23', () => {
  const { Shell } = loadShellOnly();
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  ...
  assert.equal(hero.children.length, 8, 'Hero harus tetap 8 anak seperti V2.17, tidak disentuh V2.23');
  assert.equal(cards.children.length, 8, 'Summary Cards harus tetap 8 anak seperti V2.18, tidak disentuh V2.23');
  assert.equal(moduleGrid.children.length, 10, 'Module Grid harus tetap 10 anak seperti V2.19, tidak disentuh V2.23');
  assert.equal(statisticsPanel.children.length, 8, 'Statistics Panel harus tetap 8 anak seperti V2.20, tidak disentuh V2.23');
  assert.equal(recentActivity.children.length, 9, 'Recent Activity harus tetap 9 anak seperti V2.21, tidak disentuh V2.23');
  assert.equal(upcomingTasks.children.length, 9, 'Upcoming Tasks harus tetap 9 anak seperti V2.22, tidak disentuh V2.23');
});

for (const file of ['index.html', 'app_production.html']) {
  test(`${file}: tetap 0 markup Dashboard V2 (Notifications Data Integration tetap self-mounting via JS)`, () => {
