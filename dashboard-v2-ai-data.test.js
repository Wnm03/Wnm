...
// Tahap V2.25 — AI Command Center Data Integration (lihat DASHBOARD-V2-
// AI-DATA.md).
//
...
// V2.24 (Automation Center). 4 elemen baru ditambah sbg anak AI Command
// Center, satu per fungsi adapter (getFinanceSummary/getVehicleSummary/
// getFamilySummary/getDocumentSummary), dgn fallback placeholder kalau
// adapter tidak tersedia/return `null`. 6 anak AI Command Center lama
// (1 search + 4 action card + 1 suggestion area, Tahap V2.10) TIDAK
// berubah.
//
...
// sama sekali (mensimulasikan "adapter belum tersedia"). AI Command
...
function getAICommandCenter(Shell) {
...
test('AI Command Center: adapter tidak di-load sama sekali -> 4 elemen data summary tetap ada dgn fallback placeholder', () => {
  const { Shell } = loadShellOnly();
  const section = getAICommandCenter(Shell);
  assert.equal(section.children.length, 10, 'AI Command Center harus 10 anak (6 lama V2.10 + 4 baru V2.25)');

  const finance = section.children.find((c) => c.id === 'dashboardV2AIFinanceData');
  assert.ok(finance, 'Finance summary tidak ditemukan');
  assert.equal(finance.className, 'dashboard-v2-ai-action-card');
  assert.match(finance.textContent, /placeholder/i);
  assert.match(finance.getAttribute('aria-label'), /placeholder/i);

  const vehicle = section.children.find((c) => c.id === 'dashboardV2AIVehicleData');
  assert.ok(vehicle, 'Vehicle summary tidak ditemukan');
  assert.equal(vehicle.className, 'dashboard-v2-ai-action-card');
  assert.match(vehicle.textContent, /placeholder/i);

  const family = section.children.find((c) => c.id === 'dashboardV2AIFamilyData');
  assert.ok(family, 'Family summary tidak ditemukan');
  assert.equal(family.className, 'dashboard-v2-ai-action-card');
  assert.match(family.textContent, /placeholder/i);

  const documentEl = section.children.find((c) => c.id === 'dashboardV2AIDocumentData');
  assert.ok(documentEl, 'Document summary tidak ditemukan');
  assert.equal(documentEl.className, 'dashboard-v2-ai-action-card');
  assert.match(documentEl.textContent, /placeholder/i);
});

test('AI Command Center: 6 anak lama (search/4 action card/suggestion, Tahap V2.10) tidak berubah', () => {
  const { Shell } = loadShellOnly();
  const section = getAICommandCenter(Shell);
  const [search, analyzeFinance, analyzeVehicle, generateReport, smartAssistant, suggestion] = section.children;
  ...
  assert.equal(suggestion.tagName, 'DIV');
});
...
test('AI Command Center: fungsi adapter tersedia tapi return null -> fallback placeholder (tidak error)', () => {
  const { Shell } = loadShellOnly({
  ...
  for (const id of ['dashboardV2AIFinanceData', 'dashboardV2AIVehicleData', 'dashboardV2AIFamilyData', 'dashboardV2AIDocumentData']) {
...
test('AI Command Center: getFinanceSummary() tersedia & ada data -> Finance summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('AI Command Center: getVehicleSummary() tersedia & ada data -> Vehicle summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('AI Command Center: getFamilySummary() tersedia & ada data -> Family summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('AI Command Center: getDocumentSummary() tersedia & ada data -> Document summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Integrasi sungguhan: adapter asli + D tiruan -> AI Command Center menampilkan ringkasan dari D', () => {
  const D = minimalD({
...
test('Integrasi sungguhan: render() tetap idempotent (AI Command Center 10 anak, tidak menumpuk saat dipanggil ulang)', () => {
  const D = minimalD();
  const { Shell } = loadShellWithRealAdapter(D);
  Shell.render();
  const section = getAICommandCenter(Shell);
...
test('AI Command Center: 4 elemen data summary baru semuanya punya aria-label', () => {
  const { Shell } = loadShellOnly();
  const section = getAICommandCenter(Shell);
  for (const id of ['dashboardV2AIFinanceData', 'dashboardV2AIVehicleData', 'dashboardV2AIFamilyData', 'dashboardV2AIDocumentData']) {
    const el = section.children.find((c) => c.id === id);
...
test('dashboard-v2-shell.js (setelah V2.25) tetap tidak fetch/routing/FEATURE_REGISTRY, tidak membaca D langsung', () => {
...
test('dashboard-v2-data-adapter.js TIDAK diubah tahap ini (identik dgn isi baseline V2.16..V2.24)', () => {
...
test('Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19), Statistics Panel (V2.20), Recent Activity (V2.21), Upcoming Tasks (V2.22), Notifications (V2.23) & Automation Center (V2.24) tidak ikut berubah oleh V2.25', () => {
  const { Shell } = loadShellOnly();
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  ...
  assert.equal(hero.children.length, 8, 'Hero harus tetap 8 anak seperti V2.17, tidak disentuh V2.25');
  assert.equal(cards.children.length, 8, 'Summary Cards harus tetap 8 anak seperti V2.18, tidak disentuh V2.25');
  assert.equal(moduleGrid.children.length, 10, 'Module Grid harus tetap 10 anak seperti V2.19, tidak disentuh V2.25');
  assert.equal(statisticsPanel.children.length, 8, 'Statistics Panel harus tetap 8 anak seperti V2.20, tidak disentuh V2.25');
  assert.equal(recentActivity.children.length, 9, 'Recent Activity harus tetap 9 anak seperti V2.21, tidak disentuh V2.25');
  assert.equal(upcomingTasks.children.length, 9, 'Upcoming Tasks harus tetap 9 anak seperti V2.22, tidak disentuh V2.25');
  assert.equal(notifications.children.length, 9, 'Notifications harus tetap 9 anak seperti V2.23, tidak disentuh V2.25');
  assert.equal(automationCenter.children.length, 9, 'Automation Center harus tetap 9 anak seperti V2.24, tidak disentuh V2.25');
});

for (const file of ['index.html', 'app_production.html']) {
  test(`${file}: tetap 0 markup Dashboard V2 (AI Command Center Data Integration tetap self-mounting via JS)`, () => {
