...
// Tahap V2.27 — Predictive Insights Data Integration (lihat DASHBOARD-V2-
// PREDICTIVE-DATA.md).
//
...
// (Automation Center), V2.25 (AI Command Center) & V2.26 (Health Score). 4
// elemen baru ditambah sbg anak Predictive Insights, satu per fungsi
...
// tersedia/return `null`. 5 anak Predictive Insights lama (5 kartu insight
// prediktif, Tahap V2.12) TIDAK berubah.
//
// Fake DOM sama persis dgn tests/dashboard-v2-health-data.test.js
...
// V2.21/V2.22/V2.23/V2.24/V2.25/V2.26.
...
// sama sekali (mensimulasikan "adapter belum tersedia"). Predictive
// Insights harus tetap utuh & fallback ke placeholder, bukan error.
...
function getPredictiveSection(Shell) {
...
//    placeholder, 5 kartu lama tidak berubah.
// ---------------------------------------------------------------------------

test('Predictive Insights: adapter tidak di-load sama sekali -> 4 elemen data summary tetap ada dgn fallback placeholder', () => {
  const { Shell } = loadShellOnly();
  const section = getPredictiveSection(Shell);
  assert.equal(section.children.length, 9, 'Predictive Insights harus 9 anak (5 kartu lama V2.12 + 4 baru V2.27)');

  const finance = section.children.find((c) => c.id === 'dashboardV2PredictiveFinanceData');
  assert.ok(finance, 'Finance summary tidak ditemukan');
  assert.equal(finance.className, 'dashboard-v2-predictive-data-card');
  assert.match(finance.textContent, /placeholder/i);
  assert.match(finance.getAttribute('aria-label'), /placeholder/i);

  const vehicle = section.children.find((c) => c.id === 'dashboardV2PredictiveVehicleData');
  assert.ok(vehicle, 'Vehicle summary tidak ditemukan');
  assert.equal(vehicle.className, 'dashboard-v2-predictive-data-card');
  assert.match(vehicle.textContent, /placeholder/i);

  const family = section.children.find((c) => c.id === 'dashboardV2PredictiveFamilyData');
  assert.ok(family, 'Family summary tidak ditemukan');
  assert.equal(family.className, 'dashboard-v2-predictive-data-card');
  assert.match(family.textContent, /placeholder/i);

  const documentEl = section.children.find((c) => c.id === 'dashboardV2PredictiveDocumentData');
  assert.ok(documentEl, 'Document summary tidak ditemukan');
  assert.equal(documentEl.className, 'dashboard-v2-predictive-data-card');
  assert.match(documentEl.textContent, /placeholder/i);
});

test('Predictive Insights: 5 kartu lama (Tahap V2.12) tidak berubah', () => {
  const { Shell } = loadShellOnly();
  const section = getPredictiveSection(Shell);
  const [cashFlowForecast, budgetTrend, vehicleMaintenance, familySchedule, documentExpiration] = section.children;
  assert.equal(cashFlowForecast.id, 'dashboardV2PredictiveInsightsCardCashFlowForecast');
  assert.equal(cashFlowForecast.tagName, 'BUTTON');
  assert.equal(budgetTrend.id, 'dashboardV2PredictiveInsightsCardBudgetTrend');
  assert.equal(budgetTrend.tagName, 'BUTTON');
  assert.equal(vehicleMaintenance.id, 'dashboardV2PredictiveInsightsCardVehicleMaintenancePrediction');
  assert.equal(vehicleMaintenance.tagName, 'BUTTON');
  assert.equal(familySchedule.id, 'dashboardV2PredictiveInsightsCardFamilySchedulePrediction');
  assert.equal(familySchedule.tagName, 'BUTTON');
  assert.equal(documentExpiration.id, 'dashboardV2PredictiveInsightsCardDocumentExpirationPrediction');
  assert.equal(documentExpiration.tagName, 'BUTTON');
});
...
test('Predictive Insights: fungsi adapter tersedia tapi return null -> fallback placeholder (tidak error)', () => {
  const { Shell } = loadShellOnly({
  ...
  for (const id of ['dashboardV2PredictiveFinanceData', 'dashboardV2PredictiveVehicleData', 'dashboardV2PredictiveFamilyData', 'dashboardV2PredictiveDocumentData']) {
...
test('Predictive Insights: getFinanceSummary() tersedia & ada data -> Finance summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Predictive Insights: getVehicleSummary() tersedia & ada data -> Vehicle summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Predictive Insights: getFamilySummary() tersedia & ada data -> Family summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Predictive Insights: getDocumentSummary() tersedia & ada data -> Document summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Integrasi sungguhan: adapter asli + D tiruan -> Predictive Insights menampilkan ringkasan dari D', () => {
  const D = minimalD({
...
test('Integrasi sungguhan: render() tetap idempotent (Predictive Insights 9 anak, tidak menumpuk saat dipanggil ulang)', () => {
  const D = minimalD();
  const { Shell } = loadShellWithRealAdapter(D);
  Shell.render();
  const section = getPredictiveSection(Shell);
...
test('Predictive Insights: 4 elemen data summary baru semuanya punya aria-label', () => {
  const { Shell } = loadShellOnly();
  const section = getPredictiveSection(Shell);
  for (const id of ['dashboardV2PredictiveFinanceData', 'dashboardV2PredictiveVehicleData', 'dashboardV2PredictiveFamilyData', 'dashboardV2PredictiveDocumentData']) {
    const el = section.children.find((c) => c.id === id);
...
test('dashboard-v2-shell.js (setelah V2.27) tetap tidak fetch/routing/FEATURE_REGISTRY, tidak membaca D langsung', () => {
...
test('dashboard-v2-data-adapter.js TIDAK diubah tahap ini (identik dgn isi baseline V2.16..V2.26)', () => {
...
test("dashboard-v2-shell.js: 4 fungsi adapter dipanggil lewat guard typeof (Hero V2.17 + Summary Cards V2.18 + Module Grid V2.19 + Statistics Panel V2.20 + Recent Activity V2.21 + Upcoming Tasks V2.22 + Notifications V2.23 + Automation Center V2.24 + AI Command Center V2.25 + Health Score V2.26 + Predictive Insights V2.27 = 11x per fungsi)", () => {
...
test('Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19), Statistics Panel (V2.20), Recent Activity (V2.21), Upcoming Tasks (V2.22), Notifications (V2.23), Automation Center (V2.24), AI Command Center (V2.25) & Health Score (V2.26) tidak ikut berubah oleh V2.27', () => {
  const { Shell } = loadShellOnly();
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  ...
  const healthScore = main.children.find((c) => c.id === 'dashboardV2HealthScore');
  assert.equal(hero.children.length, 8, 'Hero harus tetap 8 anak seperti V2.17, tidak disentuh V2.27');
  assert.equal(cards.children.length, 8, 'Summary Cards harus tetap 8 anak seperti V2.18, tidak disentuh V2.27');
  assert.equal(moduleGrid.children.length, 10, 'Module Grid harus tetap 10 anak seperti V2.19, tidak disentuh V2.27');
  assert.equal(statisticsPanel.children.length, 8, 'Statistics Panel harus tetap 8 anak seperti V2.20, tidak disentuh V2.27');
  assert.equal(recentActivity.children.length, 9, 'Recent Activity harus tetap 9 anak seperti V2.21, tidak disentuh V2.27');
  assert.equal(upcomingTasks.children.length, 9, 'Upcoming Tasks harus tetap 9 anak seperti V2.22, tidak disentuh V2.27');
  assert.equal(notifications.children.length, 9, 'Notifications harus tetap 9 anak seperti V2.23, tidak disentuh V2.27');
  assert.equal(automationCenter.children.length, 9, 'Automation Center harus tetap 9 anak seperti V2.24, tidak disentuh V2.27');
  assert.equal(aiCommandCenter.children.length, 10, 'AI Command Center harus tetap 10 anak seperti V2.25, tidak disentuh V2.27');
  assert.equal(healthScore.children.length, 10, 'Health Score harus tetap 10 anak seperti V2.26, tidak disentuh V2.27');
});

for (const file of ['index.html', 'app_production.html']) {
  test(`${file}: tetap 0 markup Dashboard V2 (Predictive Insights Data Integration tetap self-mounting via JS)`, () => {
