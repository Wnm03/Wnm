# Dashboard V2 — Tahap V2.22 (Upcoming Tasks Data Integration)

Baseline: `node --test` 1714/1714 PASS (akhir Tahap V2.21 — Recent
Activity Data Integration).
Hasil tahap ini: `node --test` **1732/1732 PASS** (+18 test baru, 0
regresi).

Referensi: instruksi sesi ini ("V2.22 — Upcoming Tasks Data
Integration"), `DASHBOARD-V2-RECENT-ACTIVITY-DATA.md` (V2.21, pola
yang diikuti), `DASHBOARD-V2-STATISTICS-DATA.md` (V2.20),
`DASHBOARD-V2-MODULE-GRID-DATA.md` (V2.19), `DASHBOARD-V2-SUMMARY-
DATA.md` (V2.18), `DASHBOARD-V2-HERO-DATA.md` (V2.17), `DASHBOARD-V2-
DATA-ADAPTER.md` (V2.16), Tahap V2.8 (Upcoming Tasks awal).

## Tujuan

Dashboard V2 mulai memakai `dashboard-v2-data-adapter.js` (V2.16) di
Upcoming Tasks, mengikuti pola persis Tahap V2.17 (Hero), V2.18
(Summary Cards), V2.19 (Module Grid), V2.20 (Statistics Panel) & V2.21
(Recent Activity). Bagian lain (Hero, Summary Cards, Quick Actions,
...
Automation Center) TIDAK disentuh sama sekali.

## Yang diubah

**Satu fungsi disentuh: `_buildUpcomingTasks()` di
`dashboard-v2-shell.js`.** Tidak ada fungsi lain yang diedit.

### Upcoming Tasks — sebelum (Tahap V2.8, 5 anak, TIDAK berubah)

| Elemen | id | Isi |
|---|---|---|
| Kartu Bayar Listrik | `dashboardV2UpcomingTaskCardListrik` | `<button disabled>` placeholder, 4 sub-elemen (icon/title/due date/status) |
| Kartu Servis Kendaraan | `dashboardV2UpcomingTaskCardServis` | idem |
| Kartu Backup Data | `dashboardV2UpcomingTaskCardBackup` | idem |
| Kartu Review Laporan | `dashboardV2UpcomingTaskCardLaporan` | idem |
| Kartu Perbarui Dokumen | `dashboardV2UpcomingTaskCardDokumen` | idem |

### Upcoming Tasks — ditambah tahap ini (4 anak baru, additive)

| Elemen | id | Sumber | Isi kalau adapter tersedia & ada data | Isi fallback |
|---|---|---|---|---|
| Finance summary | `dashboardV2UpcomingTasksFinanceData` | `getFinanceSummary()` | `Keuangan: {accountCount} akun, Rp {totalBalance}, {transactionCount} transaksi` | `Keuangan: -- (placeholder)` |
| Vehicle summary | `dashboardV2UpcomingTasksVehicleData` | `getVehicleSummary()` | `Kendaraan: {vehicleCount} kendaraan, {bbmLogCount} catatan BBM, {servisLogCount} catatan servis` | `Kendaraan: -- (placeholder)` |
| Family summary | `dashboardV2UpcomingTasksFamilyData` | `getFamilySummary()` | `Keluarga: {anakCount} anak, {milestoneDoneCount}/{milestoneTotalCount} milestone, {reminderCount} pengingat` | `Keluarga: -- (placeholder)` |
| Document summary | `dashboardV2UpcomingTasksDocumentData` | `getDocumentSummary()` | `Dokumen: {simCount} SIM, {vehicleTaxDocCount} dokumen pajak kendaraan` | `Dokumen: -- (placeholder)` |

Upcoming Tasks jadi total **9 anak** (5 lama + 4 baru). Urutan: kartu
listrik, servis, backup, laporan, dokumen, financeData, vehicleData,
familyData, documentData — dirender lewat `section.replaceChildren()`
seperti sebelumnya, tidak ada `innerHTML`. 4 elemen baru dibuat dengan
`document.createElement('div')` dan memakai
`className: 'dashboard-v2-upcoming-task-card'` yang sama dgn 5 kartu
lama (yang masing-masing `<button type="button" disabled>`) — konsisten
dgn pola yang dipakai `_buildStatisticsPanel()` (V2.20) &
`_buildRecentActivity()` (V2.21), di mana elemen data baru berbentuk
`<div>` polos (bukan `<button>`), karena murni menampilkan teks
ringkasan tanpa interaksi apa pun.
...
sekali di halaman), elemen jatuh ke teks placeholder. Shell **tidak
...
```
D  ──(baca-saja)──▶  dashboard-v2-data-adapter.js  ──(baca-saja, guard typeof)──▶  dashboard-v2-shell.js (_buildHero + _buildSummaryCards + _buildModuleGrid + _buildStatisticsPanel + _buildRecentActivity + _buildUpcomingTasks)
```

Setelah tahap ini, ke-4 fungsi adapter masing-masing dipanggil lewat
guard `typeof` di **6 titik** di `dashboard-v2-shell.js`: Hero (V2.17,
tidak berubah), Summary Cards (V2.18, tidak berubah), Module Grid
(V2.19, tidak berubah), Statistics Panel (V2.20, tidak berubah),
Recent Activity (V2.21, tidak berubah), dan Upcoming Tasks (V2.22,
baru). `dashboard-v2-data-adapter.js` sendiri tetap 1 lapisan tunggal
yang sama, dipanggil dari 6 tempat berbeda tanpa duplikasi logic.

## Constraint yang dijaga (diverifikasi test)

- **Additive** — 5 kartu Upcoming Tasks lama (V2.8) tidak diedit satu
  baris pun; Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19),
  Statistics Panel (V2.20) dan Recent Activity (V2.21) serta
  ...
  (diverifikasi `diff` byte-identik terhadap baseline
  V2.16/V2.17/V2.18/V2.19/V2.20/V2.21 + test tanda tangan API: tetap
  persis 4 fungsi publik + 1 guard internal, tanpa `let`/`var`
  top-level baru).
- **Tanpa fetch** — tidak ada token `fetch(` di kode aktif
  `dashboard-v2-shell.js` (diverifikasi test regex).
- **Tanpa business logic baru** — Upcoming Tasks murni menginterpolasi
...
- **Tanpa state baru** — tidak ada property `this.*` instance baru di
  ...
  `_buildUpcomingTasks()`, sama seperti 5 kartu lama.
- **Tanpa mengubah adapter** — `dashboard-v2-data-adapter.js` tidak
  disentuh sama sekali tahap ini; shell hanya *memanggilnya*, dari 6
...
- 2 assertion di `tests/dashboard-v2-upcoming.test.js` yang menghitung
  `section.children.length` disesuaikan dari `5` menjadi `9`,
  satu-satunya alasan: jumlah anak Upcoming Tasks bertambah dari
  penambahan additive tahap ini.
- 1 assertion di `tests/dashboard-v2-summary.test.js`
  (`upcomingTasks.children.length`) disesuaikan dari `5` menjadi `9`,
  alasan yang sama.
- 4 assertion constraint (guard-count per fungsi adapter) di
  ...
  `tests/dashboard-v2-recent-activity-data.test.js` diperbarui dari
  "tepat 5x" menjadi "tepat 6x", karena `_buildUpcomingTasks()`
...
1. Adapter tidak di-load sama sekali → 4 elemen baru tetap ada dgn
   fallback placeholder (Upcoming Tasks 9 anak).
2. 5 kartu lama (listrik/servis/backup/laporan/dokumen) tidak berubah.
...
12–14. Constraint statis: tanpa `fetch(`/`showPage(`/`FEATURE_REGISTRY`/
   ...
   fungsi adapter dipanggil lewat guard `typeof` tepat 6x (Hero +
   Summary Cards + Module Grid + Statistics Panel + Recent Activity +
   Upcoming Tasks).
15. Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19),
   Statistics Panel (V2.20) & Recent Activity (V2.21) tidak ikut
   berubah oleh V2.22.
