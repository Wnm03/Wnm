...
// Tahap V2.22 — Upcoming Tasks Data Integration (lihat DASHBOARD-V2-
// UPCOMING-TASKS-DATA.md).
//
...
// (Recent Activity). 4 elemen baru ditambah sbg anak Upcoming Tasks, satu
// per fungsi adapter (getFinanceSummary/getVehicleSummary/
// getFamilySummary/getDocumentSummary), dgn fallback placeholder kalau
// adapter tidak tersedia/return `null`. 5 kartu Upcoming Tasks lama
// (listrik/servis/backup/laporan/dokumen, Tahap V2.8) TIDAK berubah.
//
// Fake DOM sama persis dgn tests/dashboard-v2-recent-activity-data.test.js
...
// sama sekali (mensimulasikan "adapter belum tersedia"). Upcoming Tasks
...
function getUpcomingTasks(Shell) {
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  return main.children.find((c) => c.id === 'dashboardV2UpcomingTasks');
}
...
test('Upcoming Tasks: adapter tidak di-load sama sekali -> 4 elemen data summary tetap ada dgn fallback placeholder', () => {
  const { Shell } = loadShellOnly();
  const section = getUpcomingTasks(Shell);
  assert.equal(section.children.length, 9, 'Upcoming Tasks harus 9 anak (5 lama V2.8 + 4 baru V2.22)');

  const finance = section.children.find((c) => c.id === 'dashboardV2UpcomingTasksFinanceData');
  assert.ok(finance, 'Finance summary tidak ditemukan');
  assert.equal(finance.className, 'dashboard-v2-upcoming-task-card');
  assert.match(finance.textContent, /placeholder/i);
  assert.match(finance.getAttribute('aria-label'), /placeholder/i);

  const vehicle = section.children.find((c) => c.id === 'dashboardV2UpcomingTasksVehicleData');
  assert.ok(vehicle, 'Vehicle summary tidak ditemukan');
  assert.equal(vehicle.className, 'dashboard-v2-upcoming-task-card');
  assert.match(vehicle.textContent, /placeholder/i);

  const family = section.children.find((c) => c.id === 'dashboardV2UpcomingTasksFamilyData');
  assert.ok(family, 'Family summary tidak ditemukan');
  assert.equal(family.className, 'dashboard-v2-upcoming-task-card');
  assert.match(family.textContent, /placeholder/i);

  const documentEl = section.children.find((c) => c.id === 'dashboardV2UpcomingTasksDocumentData');
  assert.ok(documentEl, 'Document summary tidak ditemukan');
  assert.equal(documentEl.className, 'dashboard-v2-upcoming-task-card');
  assert.match(documentEl.textContent, /placeholder/i);
});

test('Upcoming Tasks: 5 kartu lama (listrik/servis/backup/laporan/dokumen, Tahap V2.8) tidak berubah', () => {
  const { Shell } = loadShellOnly();
  const section = getUpcomingTasks(Shell);
  ...
  assert.equal(listrik.disabled, true);
...
test('Upcoming Tasks: fungsi adapter tersedia tapi return null -> fallback placeholder (tidak error)', () => {
  const { Shell } = loadShellOnly({
  ...
  for (const id of ['dashboardV2UpcomingTasksFinanceData', 'dashboardV2UpcomingTasksVehicleData', 'dashboardV2UpcomingTasksFamilyData', 'dashboardV2UpcomingTasksDocumentData']) {
...
test('Upcoming Tasks: getFinanceSummary() tersedia & ada data -> Finance summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Upcoming Tasks: getVehicleSummary() tersedia & ada data -> Vehicle summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Upcoming Tasks: getFamilySummary() tersedia & ada data -> Family summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Upcoming Tasks: getDocumentSummary() tersedia & ada data -> Document summary menampilkan ringkasannya', () => {
  const { Shell } = loadShellOnly({
...
test('Integrasi sungguhan: adapter asli + D tiruan -> Upcoming Tasks menampilkan ringkasan dari D', () => {
  const D = minimalD({
...
test('Integrasi sungguhan: render() tetap idempotent (Upcoming Tasks 9 anak, tidak menumpuk saat dipanggil ulang)', () => {
  const D = minimalD();
  const { Shell } = loadShellWithRealAdapter(D);
  Shell.render();
  const section = getUpcomingTasks(Shell);
...
test('Upcoming Tasks: 4 elemen data summary baru semuanya punya aria-label', () => {
  const { Shell } = loadShellOnly();
  const section = getUpcomingTasks(Shell);
  for (const id of ['dashboardV2UpcomingTasksFinanceData', 'dashboardV2UpcomingTasksVehicleData', 'dashboardV2UpcomingTasksFamilyData', 'dashboardV2UpcomingTasksDocumentData']) {
    const el = section.children.find((c) => c.id === id);
...
test('dashboard-v2-shell.js (setelah V2.22) tetap tidak fetch/routing/FEATURE_REGISTRY, tidak membaca D langsung', () => {
...
test('dashboard-v2-data-adapter.js TIDAK diubah tahap ini (identik dgn isi baseline V2.16..V2.21)', () => {
...
test('Hero (V2.17), Summary Cards (V2.18), Module Grid (V2.19), Statistics Panel (V2.20) & Recent Activity (V2.21) tidak ikut berubah oleh V2.22', () => {
  const { Shell } = loadShellOnly();
  const root = Shell.render();
  const main = root.children.find((c) => c.id === 'dashboardV2Main');
  ...
  assert.equal(hero.children.length, 8, 'Hero harus tetap 8 anak seperti V2.17, tidak disentuh V2.22');
  assert.equal(cards.children.length, 8, 'Summary Cards harus tetap 8 anak seperti V2.18, tidak disentuh V2.22');
  assert.equal(moduleGrid.children.length, 10, 'Module Grid harus tetap 10 anak seperti V2.19, tidak disentuh V2.22');
  assert.equal(statisticsPanel.children.length, 8, 'Statistics Panel harus tetap 8 anak seperti V2.20, tidak disentuh V2.22');
  assert.equal(recentActivity.children.length, 9, 'Recent Activity harus tetap 9 anak seperti V2.21, tidak disentuh V2.22');
});

for (const file of ['index.html', 'app_production.html']) {
  test(`${file}: tetap 0 markup Dashboard V2 (Upcoming Tasks Data Integration tetap self-mounting via JS)`, () => {
